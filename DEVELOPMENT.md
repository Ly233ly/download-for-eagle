...
把 `src` 加入 `PYTHONPATH` 后运行 `unittest discover -s tests -v`。当前 44 项覆盖无来源导入、来源匹配、同页重复来源合并、网站关闭、重复内容、Eagle 离线与等待上限、旧数据迁移、自动重试竞态与次数上限、普通配对、安装器自动配对、托盘控制、URL 清理、助手自启动和完整端到端流程。

扩展的 JavaScript 使用 `node --check` 检查；`manifest.json` 需通过 JSON 解析。`constants.py`、`pyproject.toml`、扩展清单、弹窗版本、托盘菜单和安装器版本必须同步。

## 发行结构

`release/下载中转站-0.5.0` 包含：
